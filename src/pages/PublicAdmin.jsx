  ...
  return (
    <div className="relative flex min-h-dvh bg-gray-100 dark:bg-gray-900 overflow-hidden">
      ...
      <aside className="w-20 bg-white dark:bg-gray-800 shadow-md flex flex-col items-center py-4 fixed min-h-dvh z-20">
        <NavButton
          icon={<Home className={`w-6 h-6 ${activeModule === "home" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white"}`} />}
          onClick={() => setActiveModule("home")}
          isActive={activeModule === "home"}
        />
        <NavButton
          icon={<FileText className={`w-6 h-6 ${activeModule === "policies" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white"}`} />}
          onClick={() => setActiveModule("policies")}
          isActive={activeModule === "policies"}
        />
        <NavButton
          icon={<Map className={`w-6 h-6 ${activeModule === "feedback" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white"}`} />}
          onClick={() => setActiveModule("feedback")}
          isActive={activeModule === "feedback"}
        />
        <NavButton
          icon={<Users className={`w-6 h-6 ${activeModule === "profile" ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white"}`} />}
